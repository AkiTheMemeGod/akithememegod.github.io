...
const CodeBlock = ({ node, inline, className, children, ...props }) => {
    ...
    if (!inline && match) {
        return (
            <div className="my-8 rounded-xl overflow-hidden bg-zinc-950 border border-white/5 shadow-2xl">
    ...
    return (
        <code className={clsx("px-1.5 py-0.5 rounded-md bg-white/10 text-red-300 font-mono text-sm border border-white/5", className)} {...props}>
            {children}
        </code>
    );
};
...
export function BlogPost() {
    ...
    return (
        <PageWrapper className="relative min-h-screen w-full bg-black selection:bg-red-500/30 selection:text-red-50">
            {/* Ambient Background */}
            ...
            <div className="relative z-10 max-w-4xl mx-auto px-6 md:px-12 pt-32 pb-40">
                <Link to="/blog" className="inline-flex items-center gap-2 text-zinc-500 hover:text-white mb-16 transition-colors group px-4 py-2 rounded-full hover:bg-white/5 w-fit font-mono text-xs">
                    <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
                    <span>// Back to Notes</span>
                </Link>

                <header className="mb-20 text-center md:text-left space-y-6 border-b border-white/5 pb-16">
                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-xs text-zinc-500 font-mono uppercase tracking-wider">
                    ...
                    <h1 className="text-4xl md:text-6xl font-bold text-white tracking-tight leading-[1.1] md:leading-[1.1]">
                        {post.title}
                    </h1>

                    <div className="flex flex-wrap justify-center md:justify-start gap-2 pt-2">
                        {post.tags && post.tags.map(tag => (
                            <div key={tag} className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-zinc-900 border border-white/5 text-xs text-zinc-400 font-mono">
                ...
                >
                    <div className="prose prose-invert prose-lg max-w-none 
                        ...
                        prose-blockquote:border-l-red-500/50 prose-blockquote:bg-white/5 prose-blockquote:py-2 prose-blockquote:px-6 prose-blockquote:rounded-r-lg prose-blockquote:not-italic prose-blockquote:text-white/70
